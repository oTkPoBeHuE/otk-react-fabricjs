...
export default class IText extends Text {
	constructor(props, context) {
		...
		this.isEmptyStyles = () => this.state.object && this.state.object.isEmptyStyles();
		this.setSelectionStart = index => this.state.object && this.state.object.setSelectionStart(index);
		this.setSelectionEnd = index => this.state.object && this.state.object.setSelectionEnd(index);
		this.getSelectionStyles = () => this.state.object && this.state.object.getSelectionStyles();
		this.setSelectionStyles = styles => this.state.object && this.state.object.setSelectionStyles(styles);
		this.renderCursorOrSelection = () => this.state.object && this.state.object.renderCursorOrSelection();
		this.get2DCursorLocation = selectionStart =>
			this.state.object && this.state.object.get2DCursorLocation(selectionStart);
		this.getCurrentCharStyle = (lineIndex, charIndex) =>
			this.state.object && this.state.object.getCurrentCharStyle(lineIndex, charIndex);
		this.getCurrentCharFontSize = (lineIndex, charIndex) =>
			this.state.object && this.state.object.getCurrentCharFontSize(lineIndex, charIndex);
		this.getCurrentCharColor = (lineIndex, charIndex) =>
			this.state.object && this.state.object.getCurrentCharColor(lineIndex, charIndex);
		this.renderCursor = (boundaries, ctx) => this.state.object && this.state.object.renderCursor(boundaries, ctx);
		this.renderSelection = (chars, boundaries, ctx) =>
			this.state.object && this.state.object.renderSelection(chars, boundaries, ctx);
